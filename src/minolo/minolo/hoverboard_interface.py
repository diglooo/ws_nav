...
class hoverboard_node(Node):
    ...
    def __init__(self):
        ...
        self._set_vels(0,0)
        self.timer_read = self.create_timer(self.time_period, self._read_data)
        self.timer_write = self.create_timer(self.time_period, self._write_data)
        self.ser_port  =serial.Serial('/dev/ttyUSB1',115200) 
        self.get_logger().info('port opened successfully')
        self.battery_volt = 0
        self.timeout_vel = self.create_timer(2*self.time_period, self._clear_vel)
        ##ROS STYLE
